...
contract MPBondDepository is Ownable, ReentrancyGuard {
    ...
    address public immutable MP; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints MP when receives principle
    ...
    uint public constant fixedVestingTerm = 3600 * 24 * 3;
    ...
    struct Bond {
        uint payout; // MP remaining to be paid
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        if ( _parameter == PARAMETER.VESTING ) { // 0
            require(false, "cannot change the vesting term from 3 days!" );
    ...
    function deposit( 
        ...
        require( block.timestamp >= ITreasury( treasury ).bondStartTime(), "Bonding hasn't started yet!" );
        require( block.timestamp < ITreasury( treasury ).endTime() - fixedVestingTerm, "No depositing after 3 days before end time!" );
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 MP ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) MP
         */
        ...
        if ( useStableValuation ) {
            ...
            if ( fee != 0 ) { // fee is transferred to dao 
                IERC20( MP ).safeTransfer( DAO, fee ); 
            }
        } else {
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( MP ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( MP ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( MP ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( MP ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to MP supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( MP ).totalSupply();
    ...
    /**
     *  @notice calculate amount of MP available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or MP) to the DAO
     *  @return bool
     */
