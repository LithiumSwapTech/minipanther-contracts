...
contract Distributor is Policy {
    ...
    constructor( address _treasury, address _MP, uint32 _nextEpochTime ) {        
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        if ( block.timestamp > ITreasury( treasury ).endTime() )
            return 0;

        return IERC20( MP ).totalSupply().mul( _rate ).div( 1000000 );
    }
